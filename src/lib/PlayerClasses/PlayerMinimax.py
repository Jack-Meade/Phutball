...
class PlayerMinimax(Player):
    ...
    def take_turn(self, board, player1, depth, heuristic, abp):
        ...
        self._root.value = PlayerMinimax._minimax(self._root, depth, float('-inf'), float('inf'), player1, heuristic, abp)
    ...
    def _find_child(self, board):
        if len(self._root.children):
            ...
            self._root.children = [Node(cboard) for cboard in self._root.board.get_successors()]
            return self._find_child(board)

    @staticmethod
    def _minimax(node, depth, alpha, beta, player1, heuristic, abp):
        if depth == 0 or node.is_terminal():
            return node.value or heuristic(node.board)

        else:
            highlow       = float('-inf') if player1            else float('inf')
            node.children = node.children if len(node.children) else [Node(cboard) for cboard in node.board.get_successors()]

            for child in node.children:
                child.value = PlayerMinimax._minimax(child, depth-1, alpha, beta, not player1, heuristic, abp)
